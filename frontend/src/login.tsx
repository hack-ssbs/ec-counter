...
const LoginForm: React.FC = () => {
  ...
  const do_login = () => {
    fetch(`${API_PATH}/login?name=${username}&password=${password}`).then((res) => {
      console.log(res);
      if (res.status === 200) {
        res.json().then((data) => {
          console.log(data);
  ...
  const do_register = () => {
    ...
    fetch(`${API_PATH}/register?name=${username}&password=${password}&authcode=${authcode}`,{
      method: "POST"
    }).then((res) => {
      console.log(res);
      if (res.status === 200) {
      ...
      <div className="logincontainer mt-32 mx-auto">
        <div className="form-box">
          <div className="register-form hidden">
            <h3>REGISTER</h3>
            <input type="text" className='field' placeholder="username" value={username} onChange={(e) => setUsername(e.target.value)} />
            <input type="password" className='field' placeholder="password" value={password} onChange={(e) => setPassword(e.target.value)} />
            <input type="password" className='field' placeholder="confirm password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} />
            <input type="text" className='field' placeholder="auth code" value={authcode} onChange={(e) => setAuthcode(e.target.value)} />
            <button onClick={do_register}>REGISTER</button>
          </div>

          <div className="login-form">
            <h2>LOGIN</h2>
            <input type="text" className='field' name="username" placeholder="username" value={username} onChange={(e) => setUsername(e.target.value)} />
            <input type="password" className='field' name="password" placeholder="password" value={password} onChange={(e) => setPassword(e.target.value)} />
